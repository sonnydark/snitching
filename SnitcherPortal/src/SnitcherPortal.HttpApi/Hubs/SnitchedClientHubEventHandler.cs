...
public class SnitchedClientHubEventHandler : ILocalEventHandler<KillCommandEto>, ILocalEventHandler<ShowMessageEto>, ILocalEventHandler<HideMessageEto>, ITransientDependency
{
    ...
    public async Task HandleEventAsync(HideMessageEto eventData)
    {
        try
        {
            await _hubContext.Clients.Client(eventData.ConnectionId!).HideMessageReceived(eventData);
        }
        catch (Exception ex)
        {
            _logger.LogException(ex);
